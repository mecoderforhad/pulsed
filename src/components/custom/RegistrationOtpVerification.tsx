...
const RegistrationOtpVerification = ({
  ...
  const handleVerify = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_BASE_URL}/auth/register/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: data?.name, phoneNumber: data?.phone, password: data?.password, otp: otpCode,  }),
      });
    } catch (error) {
